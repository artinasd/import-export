      ...
      <div className={`container-site transition-all duration-500 ${scrolled ? "rounded-full border border-black/10 bg-[#f3f2ed]/90 shadow-lg shadow-black/5 backdrop-blur-xl" : ""}`}>
        <div className="flex h-20 items-center justify-between px-1 sm:px-3">
          <a href="#top" className="group flex items-center gap-3" aria-label="راه تجارت">
            <span className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm transition group-hover:bg-[#171816] group-hover:text-white ${scrolled ? "border-black/15 text-[#171816]" : "border-white/25 text-white"}`}>ر</span>
            <span className={`text-sm font-bold tracking-tight ${scrolled ? "text-[#171816]" : "text-white"}`}>راه تجارت</span>
          </a>

          <nav className="hidden items-center gap-8 lg:flex" aria-label="منوی اصلی">
            {links.map(([label, href]) => (
              <a key={href} href={href} className={`text-sm transition ${scrolled ? "text-black/55 hover:text-black" : "text-white/70 hover:text-white"}`}>
                {label}
              </a>
            ))}
          </nav>
        ...
        {open && (
          <div className="pb-3 lg:hidden">
            <nav className="rounded-3xl border border-black/10 bg-[#f4f3ef] p-3 shadow-2xl" aria-label="منوی موبایل">
              {links.map(([label, href]) => (
                <a key={href} href={href} onClick={() => setOpen(false)} className="block rounded-2xl px-5 py-4 text-sm font-bold hover:bg-black/5">{label}</a>
              ))}
