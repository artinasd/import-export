...
export default function Header() {
  ...
  const collaborationBadge = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "4px 12px", borderRadius: "8px", backgroundColor: "#c9a66b", border: "1px solid rgba(152,120,63,0.45)", color: "#171816", fontSize: "15px", fontWeight: 800, lineHeight: "22px", whiteSpace: "nowrap", boxSizing: "border-box" };
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "py-3" : "py-0"}`}>
      <div className={`container-site transition-all duration-500 ${scrolled ? "rounded-full border border-black/10 bg-[#f3f2ed]/90 shadow-lg shadow-black/5 backdrop-blur-xl" : ""}`}>
        <div className="flex h-20 items-center justify-between px-1 sm:px-3">
          ...
          <a href="#inquiry" style={{ display: "none", textDecoration: "none" }} className="lg:inline-flex" aria-label="درخواست همکاری"><span style={collaborationBadge}>درخواست همکاری <ArrowLeft size={15} /></span></a>
