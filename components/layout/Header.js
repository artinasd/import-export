...
const links = [["خانه", "/#top"], ["خدمات", "/#services"], ["دانش تجارت", "/trade-knowledge"], ["درباره ما", "/about"], ["تماس با ما", "/contact"]];
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "py-3" : "py-0"}`}>
      <div className={`container-site transition-all duration-500 ${scrolled ? "rounded-full border border-black/10 bg-[#f3f2ed]/90 shadow-lg shadow-black/5 backdrop-blur-xl" : ""}`}>
        <div className="grid h-20 grid-cols-[1fr_auto_1fr] items-center px-1 sm:px-3">
          <a href="/#top" className="group flex items-center gap-3 justify-self-start" aria-label="راه تجارت"><span className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm transition group-hover:bg-[#171816] group-hover:text-white ${scrolled ? "border-black/15 text-[#171816]" : "border-white/25 text-white"}`}>ر</span><span className={`text-sm font-bold tracking-tight ${scrolled ? "text-[#171816]" : "text-white"}`}>راه تجارت</span></a>
          ...
          <a href="/inquiry" className="hidden lg:inline-flex justify-self-end" aria-label="درخواست همکاری"><span style={labelStyle}>درخواست همکاری <ArrowLeft size={15} /></span></a>
          <button type="button" onClick={() => setOpen((value) => !value)} className={`flex h-11 w-11 items-center justify-center rounded-full border lg:hidden justify-self-end ${scrolled ? "border-black/10 text-black" : "border-white/20 text-white"}`} aria-label={open ? "بستن منو" : "باز کردن منو"} aria-expanded={open}>{open ? <X size={20} /> : <Menu size={20} />}</button>
        </div>
