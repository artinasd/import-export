  ...
  return (
    <section className="bg-[#f3f2ed] py-20 lg:py-28">
      <div className="container-site">
        ...
        <div className="mt-12 grid border-t border-black/10 md:grid-cols-2 lg:grid-cols-3">{filtered.map(([itemCategory, title, text], index) => <article key={title} className={`flex min-h-[310px] flex-col border-b border-black/10 p-7 lg:p-9 ${index % 3 !== 2 ? "lg:border-l" : ""}`}><div className="flex items-center justify-between text-xs text-black/40"><span>{itemCategory}</span><span>۰{(index % 9) + 1}</span></div><div className="mt-auto"><h3 className="text-xl font-black leading-8">{title}</h3><p className="mt-4 text-sm leading-7 text-[#686963]">{text}</p><a href="#" className="mt-7 inline-flex items-center gap-2 text-xs font-bold">مطالعه مطلب<ArrowUpLeft size={14} /></a></div></article>)}</div>
